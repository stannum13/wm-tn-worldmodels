...
def run(seeds: int, train_streams: int, evaluation_streams: int, length: int) -> dict[str, object]:
    ...
    raw: dict[str, dict[str, list[dict[str, float]]]] = {
        str(delay): {name: [] for name in ["instantaneous", "ewma", "hmm_filter_current", "hmm_delay_forecast", "hmm_clipped_forecast", "stationary_prior"]}
        for delay in delays
    }
    fits = []
    ...
    for delay in map(str, delays):
        ...
        best_simple = min(
            ...
            for name in ["instantaneous", "ewma", "hmm_filter_current", "hmm_clipped_forecast", "stationary_prior"]
        )
        improvements[delay] = {
